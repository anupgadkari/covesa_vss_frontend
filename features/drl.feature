Feature: Daytime Running Lights (DRL)
  ...
  #              "START" AND Vehicle.Chassis.ParkingBrake.IsEngaged is FALSE, the
  #              feature SHALL request Vehicle.Body.Lights.Running.IsOn = TRUE via
  ...
  #              the feature SHALL request Vehicle.Body.Lights.Running.IsOn = FALSE
  #              at priority MEDIUM (2).
  #
  # REQ-DRL-003: When Vehicle.LowVoltageSystemState is "ON" or "START" AND
  #              Vehicle.Chassis.ParkingBrake.IsEngaged transitions to TRUE, the
  #              feature SHALL request Vehicle.Body.Lights.Running.IsOn = FALSE
  #              at priority MEDIUM (2). (DRL off while parked.)
  #
  # REQ-DRL-004: When Vehicle.Chassis.ParkingBrake.IsEngaged transitions to FALSE
  #              while Vehicle.LowVoltageSystemState is "ON" or "START",
  #              the feature SHALL request Vehicle.Body.Lights.Running.IsOn = TRUE
  ...
  #              Vehicle.Chassis.ParkingBrake.IsEngaged (overlay). It SHALL NOT
  ...
  Scenario: Vehicle powered on, parking brake released — DRL activates
    ...
    And Vehicle.Chassis.ParkingBrake.IsEngaged is FALSE
    When Vehicle.LowVoltageSystemState transitions to "ON"
    Then the DRL feature requests Vehicle.Body.Lights.Running.IsOn = TRUE at priority MEDIUM
  ...
  Scenario: Vehicle powered off — DRL deactivates
    ...
    Then the DRL feature requests Vehicle.Body.Lights.Running.IsOn = FALSE at priority MEDIUM
  ...
  Scenario: Parking brake engaged while running — DRL deactivates
    ...
    And Vehicle.Chassis.ParkingBrake.IsEngaged is FALSE
    And DRL is active
    When Vehicle.Chassis.ParkingBrake.IsEngaged transitions to TRUE
    Then the DRL feature requests Vehicle.Body.Lights.Running.IsOn = FALSE at priority MEDIUM

  # --- REQ-DRL-004 ---
  Scenario: Parking brake released while running — DRL reactivates
    Given Vehicle.LowVoltageSystemState is "ON"
    And Vehicle.Chassis.ParkingBrake.IsEngaged is TRUE
    And DRL is off
    When Vehicle.Chassis.ParkingBrake.IsEngaged transitions to FALSE
    Then the DRL feature requests Vehicle.Body.Lights.Running.IsOn = TRUE at priority MEDIUM
  ...
  Scenario: Vehicle started with parking brake engaged
    ...
    And Vehicle.Chassis.ParkingBrake.IsEngaged is TRUE
    When Vehicle.LowVoltageSystemState transitions to "ON"
    Then the DRL feature does NOT request Vehicle.Body.Lights.Running.IsOn = TRUE
