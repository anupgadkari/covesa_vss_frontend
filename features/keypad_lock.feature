Feature: Keypad Lock
  ...
  capacitive keypad on the outside door handle for 500 ms
  ...
  #              handle keypad continuously for 500 ms SHALL issue a LockAll
  ...
  Scenario: Right pad held for 500 ms — vehicle locks
    When the Row 1 Right keypad is pressed
    And 500 ms elapse with the pad continuously held
    Then a LockAll command is issued via the DoorLockArbiter
    And FeedbackRequest "lock" is published
  ...
  Scenario: Pad released before 500 ms — no lock
    When the Row 1 Left keypad is pressed
    And 200 ms elapse
    And the Row 1 Left keypad is released
  ...
  Scenario: New press resets debounce window
    ...
    And 300 ms elapse
    And the Row 1 Left keypad is released
    And the Row 1 Left keypad is pressed again immediately
    And 500 ms elapse
  ...
  Scenario: Both pads held — single lock command fires
    When the Row 1 Left keypad is pressed
    And the Row 1 Right keypad is pressed simultaneously
    And 500 ms elapse
    Then exactly one LockAll command is issued

  # --- REQ-TPL-006 ---
  Scenario: Right pad fires while left pad has been recently released
    When the Row 1 Left keypad is pressed and released before 500 ms
    And the Row 1 Right keypad is separately pressed and held for 500 ms
    Then a LockAll command is issued (from the right pad)
    And no spurious command is issued from the cancelled left pad

  # --- REQ-TPL-007 ---
  Scenario: Row 2 doors have no keypads — no monitoring
