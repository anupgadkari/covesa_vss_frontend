Feature: Perimeter Alarm (anti-intrusion alarm on unauthorised door open)
  ...
  #   • Vehicle.Controller.Cabin.LockStatus                          — arming gate
  #   • Vehicle.Controller.Cabin.LockStatus.LastRequestor            — disarm-source identity
  #   • Vehicle.Controller.Body.Switches.Panic.IsEngaged             — disarm via panic press
  #
  # On the FALSE→TRUE edge of Vehicle.Controller.Cabin.LockStatus into LOCKED / DOUBLE_LOCKED
  ...
  #
  #   1. Chime warning (0..12 s).      Pulse Vehicle.Controller.Body.Chime.IsActive only.
  #                                    Vehicle.Controller.Alarm.IsActive stays
  #                                    FALSE so HMI/telematics know this
  #                                    is a warning, not a real alarm.
  #
  #   2. Full alarm    (12..42 s).     Vehicle.Controller.Alarm.IsActive = TRUE.
  #                                    Pulse direction indicators (both),
  #                                    dome, exterior puddle lamps, AND
  #                                    Vehicle.Body.Horn.IsActive in unison.
  #
  ...
  # Requirements
  # -------------------------------------------------------------------------
  # REQ-PERI-001: When Vehicle.Controller.Cabin.LockStatus transitions from a non-armable
  ...
  #               chime, no horn, no lights, no Vehicle.Controller.Alarm.IsActive
  ...
  #
  # REQ-PERI-003: When Vehicle.Controller.Cabin.LockStatus transitions from an armable state
  ...
  #               transitions FALSE→TRUE while Vehicle.Controller.Cabin.LockStatus is LOCKED
  ...
  #               feature SHALL pulse Vehicle.Controller.Body.Chime.IsActive at 1 Hz
  #               (400 ms ON, 600 ms OFF).  It SHALL NOT request horn,
  #               direction indicators, dome, or puddle lamps.  It SHALL
  #               NOT publish Vehicle.Controller.Alarm.IsActive — the status
  ...
  #                 • publish Vehicle.Controller.Body.Chime.IsActive = FALSE,
  #                 • publish Vehicle.Controller.Alarm.IsActive = TRUE, and
  #                 • begin pulsing direction indicators (both),
  #                   Vehicle.Cabin.Light.IsDomeOn, both Body.Lights.Puddle.*,
  #                   and Vehicle.Body.Horn.IsActive at 1 Hz, all on the same
  ...
  #               courtesy, puddle) and publish Vehicle.Controller.Body.Chime.IsActive =
  #               FALSE.  Vehicle.Controller.Alarm.IsActive is NOT auto-cleared
  #               on natural expiry — the next lock/unlock cycle clears
  #               it via the disarm path or via REQ-PERI-009.
  #
  # REQ-PERI-009: When Vehicle.Controller.Cabin.LockStatus transitions to UNLOCKED or
  #               DRIVER_UNLOCKED AND Vehicle.Controller.Cabin.LockStatus.LastRequestor is
  ...
  #               arbiter claims, publish Vehicle.Controller.Body.Chime.IsActive = FALSE, and
  #               publish Vehicle.Controller.Alarm.IsActive = FALSE.  Disarm
  ...
  #
  # REQ-PERI-011: When Vehicle.Controller.Body.Switches.Panic.IsEngaged transitions to TRUE
  ...
  #               extend, or re-publish Vehicle.Controller.Alarm.IsActive on
  ...
  #               Vehicle.Controller.Body.Chime.IsActive = FALSE, and publish
  #               Vehicle.Controller.Alarm.IsActive = FALSE.  Cranking or
  ...
  Scenario: Door open during the 20 s pre-arm window is silently ignored
    Given Vehicle.Controller.Cabin.LockStatus has just transitioned to "LOCKED"
    And less than 20 seconds have elapsed since that transition
    When the driver opens the Row1.Left door
    Then Vehicle.Controller.Alarm.IsActive remains FALSE
    And Vehicle.Controller.Body.Chime.IsActive does NOT pulse
    And Vehicle.Body.Horn.IsActive does NOT pulse
  ...
  Scenario: Door open just after the pre-arm window starts the chime phase
    Given Vehicle.Controller.Cabin.LockStatus has been "LOCKED" for at least 20 seconds
    When the driver opens the Row1.Left door
    Then Vehicle.Controller.Body.Chime.IsActive begins pulsing at 1 Hz (400 ms ON / 600 ms OFF)
    And Vehicle.Controller.Alarm.IsActive remains FALSE during the chime phase

  # --- REQ-PERI-002 ---
  Scenario: LOCKED → DOUBLE_LOCKED does not restart the pre-arm timer
    Given Vehicle.Controller.Cabin.LockStatus has been "LOCKED" for 15 seconds
    When Vehicle.Controller.Cabin.LockStatus transitions to "DOUBLE_LOCKED"
    And 6 more seconds elapse
    And the driver opens the Row1.Left door
    Then Vehicle.Controller.Body.Chime.IsActive begins pulsing
    # Total armable time = 15 + 6 = 21 s, past the pre-arm window.

  # --- REQ-PERI-003 ---
  Scenario: Unlock-then-relock resets the pre-arm window
    Given Vehicle.Controller.Cabin.LockStatus has been "LOCKED" for 15 seconds
    When Vehicle.Controller.Cabin.LockStatus transitions to "UNLOCKED"
    And Vehicle.Controller.Cabin.LockStatus transitions back to "LOCKED"
    And 6 seconds elapse since the second lock
    And the driver opens the Row1.Left door
    Then Vehicle.Controller.Body.Chime.IsActive does NOT pulse
  ...
  Scenario: Chime warning phase publishes only the chime signal
    ...
    Then Vehicle.Controller.Body.Chime.IsActive is pulsing at 1 Hz
    And Vehicle.Body.Horn.IsActive has not been published TRUE
    And Vehicle.Body.Lights.DirectionIndicator.Left.IsSignaling has not been published TRUE
    And Vehicle.Cabin.Light.IsDomeOn has not been published TRUE
    And Vehicle.Controller.Body.Lights.Puddle.Left.IsOn has not been published TRUE
    And Vehicle.Controller.Alarm.IsActive remains FALSE
  ...
  Scenario: Chime escalates to full alarm after 12 seconds
    ...
    Then Vehicle.Controller.Body.Chime.IsActive has been published FALSE
    And Vehicle.Controller.Alarm.IsActive is TRUE
    And Vehicle.Body.Horn.IsActive is pulsing at priority HIGH
    And both direction indicators are pulsing at priority HIGH
    And Vehicle.Cabin.Light.IsDomeOn is pulsing at priority HIGH
  ...
  Scenario: Horn releases 30 s after chime ends; lights continue
    ...
    And Vehicle.Body.Horn.IsActive has settled FALSE
    And the direction indicators continue pulsing
    And Vehicle.Controller.Alarm.IsActive remains TRUE
  ...
  Scenario: Pulse loop self-exits 5 minutes after the chime ends
    ...
    And Vehicle.Controller.Body.Chime.IsActive is FALSE
  ...
  Scenario: Authenticated unlock during full alarm disarms immediately
    Given a PerimeterAlarm sequence is in the full-alarm phase
    When Vehicle.Controller.Cabin.LockStatus transitions to "UNLOCKED"
    And Vehicle.Controller.Cabin.LockStatus.LastRequestor becomes "PassiveEntry"
  ...
  Scenario: Thumb-pad unlock does NOT disarm a running alarm
    Given a PerimeterAlarm sequence is in the full-alarm phase
    When Vehicle.Controller.Cabin.LockStatus transitions to "UNLOCKED"
    And Vehicle.Controller.Cabin.LockStatus.LastRequestor becomes "KeypadLock"
  ...
  Scenario: Authenticated unlock during chime phase produces no exterior output
    ...
    When Vehicle.Controller.Cabin.LockStatus transitions to "UNLOCKED"
    And Vehicle.Controller.Cabin.LockStatus.LastRequestor becomes "KeyfobRke"
    Then Vehicle.Controller.Body.Chime.IsActive is FALSE
    And Vehicle.Controller.Alarm.IsActive is FALSE
    And Vehicle.Body.Horn.IsActive has never been published TRUE during this sequence
  ...
  Scenario: ACC during chime does NOT disarm
    ...
    Then Vehicle.Controller.Body.Chime.IsActive continues pulsing
  ...
  Scenario: All pulse outputs share identical 1 Hz cadence
    ...
    Then Vehicle.Body.Horn.IsActive ON edges align with the direction indicator ON edges
